...
paired_comparison <- function(data, before_col, after_col, parametric = FALSE, plot_title = NULL, xlab = NULL, ylab = "Value", before_label = NULL, after_label = NULL, show_paired_lines = TRUE, before_color = NULL, after_color = NULL) {
  ...
  # --- THE FIX: Set the plot device dimensions before creating the plot ---
  options(webr.fig.width = 450, webr.fig.height = 350)
  ...
  stats_summary_text <- paste(
    ...
    paste("Method:", test_name),
    paste("P-value:", format.pval(stats_res$p, digits = 4, eps = 0.0001)),
    paste("Statistic (", names(stats_res$statistic), "): ", round(stats_res$statistic, 2)),
    sep = "\n"
  )
