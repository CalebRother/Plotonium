...
paired_comparison <- function(data,
  ...
  # apply custom labels & factor levels
  levels_vec <- c(before_str, after_str)
  if (!is.null(before_label) && !is.null(after_label)) {
    ...
    data_long$time <- factor(data_long$time, levels = levels_vec)
  }
  ...
  # add plotting positions for bracket
  stat.test <- stats_res %>%
  ...
  p <- ggplot(data_long, aes(x = time, y = value)) +
    ...
    { if(show_paired_lines) geom_line(aes(group = subject_id),
  ...
  cat(
    ...
    "  Method: ", test_name, "\n",
    "  Groups: ", stat.test$group1, " vs ", stat.test$group2, "\n",
    "  Statistic: ", round(stat.test$statistic, 2), "\n",
    if(!is.null(stat.test$df)) paste0("  df: ", stat.test$df, "\n") else "",
